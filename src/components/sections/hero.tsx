...
type CPPNMaterial = THREE.ShaderMaterial & {
  iTime: number;
  iResolution: THREE.Vector2;
};

function ShaderPlane() {
  const materialRef = useRef<CPPNMaterial | null>(null);
  const planeSize = useMemo(() => {
    const aspect = typeof window !== "undefined" ? window.innerWidth / window.innerHeight : 1;
    const baseSize = 6;
    const width = aspect > 2 ? baseSize * (aspect / 1.5) : baseSize;
  ...
  const meshPosition: [number, number, number] = [0, -0.75, -0.5];
...
export default function Hero() {
  ...
  useGSAP(
    () => {
      ...
      document.fonts.ready.then(() => {
        ...
        const microItems = [microItem1Ref.current, microItem2Ref.current].filter(Boolean);
  ...
  return (
    <section ref={sectionRef} className="relative min-h-[80vh] w-full overflow-hidden">
      <ShaderBackground />
      <div className="relative mx-auto flex max-w-7xl flex-col items-start gap-6 px-6 pb-24 pt-36 sm:gap-8 sm:pt-44 md:px-10 lg:px-16">
        <div ref={badgeRef} className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 backdrop-blur-sm">
          ...
          <span className="text-xs font-light tracking-tight text-white/80">OnePay</span>
        </div>
        <h1 ref={headerRef} className="max-w-2xl text-left text-5xl font-extralight leading-[1.05] tracking-tight text-white sm:text-6xl md:text-7xl">
          Next-Gen Crypto Payments Platform
        </h1>
        <p ref={paraRef} className="max-w-xl text-left text-base font-light leading-relaxed tracking-tight text-white/75 sm:text-lg">
          Seamlessly accept cryptocurrency payments with our advanced Web3 infrastructure. Built for businesses ready to embrace the future of digital transactions.
        </p>
        <div ref={ctaRef} className="flex flex-wrap items-center gap-3 pt-2">
          <a href="/product" className="rounded-2xl border border-white/10 px-5 py-3 text-sm font-light tracking-tight transition-colors focus:outline-none focus:ring-2 focus:ring-white/30 duration-300 bg-white/10 text-white backdrop-blur-sm hover:bg-white/20">
            Start Integration
          </a>
          <a href="/document" className="rounded-2xl border border-white/10 px-5 py-3 text-sm font-light tracking-tight transition-colors focus:outline-none focus:ring-2 focus:ring-white/30 duration-300 text-white/80 hover:bg-white/5">
            View Documentation
          </a>
        </div>
        <ul ref={microRef} className="mt-8 flex flex-wrap gap-6 text-xs font-extralight tracking-tight text-white/60">
          <li ref={microItem1Ref} className="flex items-center gap-2"><span className="h-1 w-1 rounded-full bg-white/40" /> Multi‑chain Support</li>
          <li ref={microItem2Ref} className="flex items-center gap-2"><span className="h-1 w-1 rounded-full bg-white/40" /> Real‑time Tracking</li>
        </ul>
      </div>
